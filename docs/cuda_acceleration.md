...
| `VMECPP_SCATTER_I8_LIMBS` | Limb width for the int8 scatter paths: `4` selects 28-bit operands (rel ~4e-9) at half the limb-plane traffic and half the mma work; the default `8` covers the FP64 mantissa. Pure 4-limb runs stall above the convergence tolerance. Under `VMECPP_IR_STAGED` the residual phase routes the width per iteration (4 above the threshold with a decade hysteresis band, 8 below; a width change drops the whole-iteration graph). Measured outcome on sm_86: the 4-limb descent inflates iteration counts enough to lose to the pure 8-limb path on every case tried (cma 0.59 s staged vs 0.27 s, N = 64 batch 21.1 s vs 5.8 s), and on the restart-heavy W7-X case the staged descent does not converge. Recorded as a negative result. |
