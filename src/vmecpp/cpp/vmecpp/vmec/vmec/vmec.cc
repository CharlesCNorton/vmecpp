...
// (VMECPP_SYNC_ELIDE=K, non-boundary iteration). Written once per
// iteration in Vmec::Evolve before UpdateForwardModel; read by the
// convergence-gate block and the restart/store bookkeeping in
// SolveEquilibriumLoop, which only evaluate on boundary iterations.
bool g_sync_elided_iter = false;

// Run-scoped environment gates, re-read at the start of every Vmec::run
// (sentinel values mark them unread) so successive runs in one process
// can carry different settings.
int g_sync_elide_k = -2;  // VMECPP_SYNC_ELIDE
#endif

// VMECPP_PER_CFG_NITER_CAP, run-scoped with the same sentinel
// convention. Declared unconditionally because the per-configuration
// convergence gate that reads it is compiled in all builds.
// NOLINTNEXTLINE(cppcoreguidelines-avoid-non-const-global-variables)
int g_per_cfg_niter_cap = -1;
...
absl::StatusOr<bool> Vmec::run(const VmecCheckpoint& checkpoint,
                               ...
                               std::optional<HotRestartState> initial_state) {
#ifdef VMECPP_USE_CUDA
  // The CUDA iteration body covers stellarator-symmetric,
  // three-dimensional configurations on a single radial rank. Reject the
  // unsupported inputs up front rather than failing mid-iteration.
  if (indata_.lasym) {
...
#pragma omp single
  ...
  vmecpp::SetSyncElideIterCuda(g_sync_elided_iter ? 1 : 0);
  ...
  if (sync_elide_k > 0 && g_sync_elided_iter &&
      vmecpp::IterGraphEnabledCuda()) {
...
#endif  // _OPENMP
  {
    ...
    if (g_sync_elided_iter) {
      ...
      if (n_cfg > 0 && static_cast<int>(fc_.fsqr_per_cfg.size()) == n_cfg &&
        ...
        // the shared niterv.
        if (g_per_cfg_niter_cap < 0) {
          const char* e = std::getenv("VMECPP_PER_CFG_NITER_CAP");
          g_per_cfg_niter_cap = (e && std::atoi(e) > 0)
                                    ? std::atoi(e)
                                    : std::numeric_limits<int>::max();
          if (g_per_cfg_niter_cap != std::numeric_limits<int>::max()) {
            std::fprintf(
                stderr,
        ...
        fc_.niter_max_per_cfg = g_per_cfg_niter_cap;
