...
retry that cfg via the operator's multiprocessing-Pool path.
  ...
  m.def(
      ...
      [](const std::vector<VmecINDATA> &indata_list,
        ...
        const bool distinct_mode =
            (distinct_env != nullptr && std::atoi(distinct_env) > 0);
      ...
      // recompute consumes.
        ...
        {
          py::gil_scoped_release release;
          ret_seed = vmecpp::run(*run_seed, std::nullopt, max_threads, verbose,
                                 interrupt_check);
        }
#ifdef VMECPP_USE_CUDA
...
the VMECPP_BATCH_DISTINCT environment variable:
  ...
  Distinct mode (VMECPP_BATCH_DISTINCT=1): a per-configuration
  pre-initialization stage extracts each input's initial spectra,
